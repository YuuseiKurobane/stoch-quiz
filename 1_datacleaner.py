# kanjiwords_jmdictslice_from_raw_v3_competition.py
#
...
#
# Filtering:
# - Drop entries whose displayValue contains '㋕'
# - Drop entries whose word is not present in JmdictFurigana headwords (text before first '|')
#
# Rank rule (what you want):
# - Reassign ranks to eliminate unused values while preserving order and ties
# - This is COMPETITION ranking (1224 ranking) after filtering:
...
def main() -> None:
    ...
    current_group_rows: List[ParsedRow] = []  # only kept rows of this rawrank
    ...
    def flush_group() -> None:
        ...
        if not current_group_rows:
            return
        # Competition ranking: all ties get same rank, next rank jumps by group size
    ...
    with open(OUTPUT_CSV, "w", encoding="utf-8", newline="") as f_out:
        ...
        for entry in iter_top_level_json_array(INPUT_JSON):
            ...
            # sanity check ordering for freq entries
            ...
            # group boundary on rawrank change (even if rows are later filtered out)
